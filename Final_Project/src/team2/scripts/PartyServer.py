...
class Global:
        ...
        for i in range(0, mapWidth):
            for j in range(0, mapHeight):
                if self.costMapGrid.data[(i * mapWidth) + j] > self.threshold:
                    ...
                    for k in range(-7, 8):
                        for l in range(-7, 8):
                            if self.staticMapGrid.data[((i + k) * mapWidth) + j + l] > 0:
                                staticValue = 1
                                break
                        else:
                            continue
    ...
    def evaluateGroups(self, potentialPoints):
        ...
        mapWidth = self.costMapGrid.info.width
        mapHeight = self.costMapGrid.info.height
        mapOriginX = int(math.floor(self.costMapGrid.info.origin.position.x * 20))
        mapOriginY = int(math.floor(self.costMapGrid.info.origin.position.y * 20))
        ...
        for n in range(0, len(self.allGroups)):
            for o in range(0, len(self.allGroups[n].cells)):
                if self.costMapGrid.data[((self.allGroups[n].cells[len(self.allGroups[n].cells) - 1 - o].y + mapOriginY) * mapWidth) + 
                                    self.allGroups[n].cells[len(self.allGroups[n].cells) - 1 - o].x + mapOriginX] < self.threshold:
                    del self.allGroups[n].cells[len(self.allGroups[n].cells) - 1 - o]
...
if __name__ == '__main__':
    ...
    while(1):
        ...
        print len(cells)
        oldData = g.costMapGrid.data
        while oldData == g.costMapGrid.data:
            rand = 1
